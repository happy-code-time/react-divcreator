import React, { Component } from 'react';

import { menuVerticalClickData, menuVerticalClickKeys, mainDescription } from '../../store/menuVerticalClick';

import TypesGenerateor from '../../modules/typesGenerator';

import { storeData, storeKeys } from '../../store/reducer';

class MenuVerticalClickWebsite extends Component {

    constructor(props) {
        super(props)
    }
    /**
     * dispatch dtata to the redux store
     */
    dispatchData(){
        storeData.dispatch({
            type: 'data',
            data : menuVerticalClickData
        });
        storeKeys.dispatch({
            type: 'keys',
            keys : menuVerticalClickKeys
        });
    }
    /**
     * Return modules data
     */
    moduleData() {
        return {
            subTitle: 'MenuVerticalClick',
            examples: storeData.getState(),
            keys : storeKeys.getState(),
            mainDescription
        };
    }
    render() {
        this.dispatchData();
        const moduleData = this.moduleData();

        return (
            <div className='divcreator-types'>
                <TypesGenerateor data={moduleData} />
            </div>
        );
    }
};
export default MenuVerticalClickWebsite;
